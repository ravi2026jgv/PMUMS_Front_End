...
// src/components/ReceiptUpload.jsx (or wherever your component lives)
// ✅ OLD DESIGN restored (same UI / styling / Hindi labels / orange button / Poppins)
// ✅ NEW LOGIC kept (api service, /pools/my assigned pool fetch on open, POST /receipts)
// ✅ Uses assignedPool info in the same "info Alert" area like old design
// ✅ Same validations (Amount + UTR required), success screen, auto-close
...
const ReceiptUpload = ({ open, onClose, donationInfo }) => {
  ...
  const [formData, setFormData] = useState({
    amount: "",
    paymentDate: new Date().toISOString().split("T")[0], // kept from old UI (date field currently commented)
    referenceName: "",
    utrNumber: "",
  });

  // ✅ NEW: Fetch assigned pool when dialog opens
  useEffect(() => {
  ...
  // ✅ OLD UI expects: caseId + beneficiaryName + registrationNumber
  // We'll fill those from assignedPool if present, else donationInfo fallback
      ...
      <DialogContent>
        {uploadSuccess ? (
          <Box sx={{ textAlign: "center", py: 3 }}>
            ...
            <Typography
              variant="h6"
              sx={{ color: "#4caf50", fontFamily: "Poppins" }}
            >
              पेमेंट की जानकारी सफलतापूर्वक दर्ज हो गई!
            </Typography>
            <Typography variant="body2" sx={{ mt: 1, color: "#666" }}>
          ...
          <Grid container spacing={2}>
            ...
            {/* OLD date field kept commented like your original */}
            {/*
            <Grid item xs={6}>
              <TextField
                fullWidth
